...
let petik = '```'
let enter = '\n'
let notnot = `NOTE:${enter}𝐉𝐚𝐧𝐠𝐚𝐧 𝐒𝐩𝐚𝐦 𝐂𝐨𝐦𝐦𝐚𝐧𝐝 𝐁𝐨𝐭!!`
...
var waktoo = date.getHours();
		...
		switch(bulan1) {
			...
			if(time2 < "23:59:00"){
				var ucapannya = `𝑵𝒊𝒈𝒉𝒕 🌚 ${pushname}`
				}
       	 if(time2 < "19:00:00"){
     		   var ucapannya = `𝑵𝒊𝒈𝒉𝒕 🌚 ${pushname}`
				}
     	   if(time2 < "18:00:00"){
     		   var ucapannya = `𝑨𝒇𝒕𝒆𝒓𝒏𝒐𝒐𝒏 🌅 ${pushname}`
				}
      	  if(time2 < "15:00:00"){
     		   var ucapannya = `𝑮𝒐𝒐𝒅𝑫𝒂𝒚 🌞 ${pushname}`
				}
    	    if(time2 < "11:00:00"){
       		 var ucapannya = `𝑴𝒐𝒓𝒏𝒊𝒏𝒈 🌄 ${pushname}`
				}
    	    if(time2 < "05:00:00"){
  		      var ucapannya = `𝑵𝒊𝒈𝒉𝒕 🌚 ${pushname}`
				}
				mess = {
					wait: '```[ ♲︎︎︎ ]``` 𝑷𝒓𝒐𝒔𝒆𝒔..',
					limit: '```[ ⚠︎ ]``` 𝑷𝒆𝒎𝒂𝒌𝒂𝒊𝒂𝒏 𝑩𝒐𝒕 𝑼𝒏𝒕𝒖𝒌 𝑲𝒂𝒎𝒖 𝑻𝒆𝒍𝒂𝒉 𝑴𝒆𝒏𝒄𝒂𝒑𝒂𝒊 𝑩𝒂𝒕𝒂𝒔...',
					sewabot: ` 「 *SEWA BOT* 」 \n\nBerikut list sewa bot kami

•> 25k / BULAN (PREMIUM)
•> 30k PERMANEN (NO PREM)

Minat? Pm wa.me/${owner_number}`,
					ban: '```[ ⚠︎ ]``` 𝑵𝒐𝒎𝒐𝒓 𝑨𝒏𝒅𝒂 𝑺𝒖𝒅𝒂𝒉 𝑫𝒊 𝑩𝒂𝒏𝒏𝒆𝒅!!',
					error: '```[ ⚠︎ ]``` 𝑬𝒓𝒓𝒐𝒓, 𝑴𝒖𝒏𝒈𝒌𝒊𝒏 𝑲𝒂𝒎𝒖 𝑾𝒊𝒃𝒖',
					errorLink: '```[ ⚠︎ ]``` 𝑳𝒊𝒏𝒌 𝑬𝒓𝒓𝒐𝒓',
					succes: '```[ √ ]``` 𝑺𝒖𝒄𝒄𝒆𝒔𝒔',
					only: {
						personal: '```[ ⚠︎ ]``` 𝑲𝒉𝒖𝒔𝒖𝒔 𝑷𝒓𝒊𝒗𝒂𝒕𝒆 𝑪𝒉𝒂𝒕',
						admin: '```[ ⚠︎ ]``` 𝑲𝒉𝒖𝒔𝒖𝒔 𝑨𝒅𝒎𝒊𝒏 𝑮𝒓𝒐𝒖𝒑',
						group: '```[ ⚠︎ ]``` 𝑲𝒉𝒖𝒔𝒖𝒔 𝑫𝒂𝒍𝒂𝒎 𝑮𝒓𝒖𝒑',
						 prem: '```[ ⚠︎ ]``` 𝑲𝒉𝒖𝒔𝒖𝒔 𝑼𝒔𝒆𝒓 𝑷𝒓𝒆𝒎𝒊𝒖𝒎',
						owner: '```[ ⚠︎ ]``` 𝑲𝒉𝒖𝒔𝒖𝒔 𝑶𝒘𝒏𝒆𝒓 𝑩𝒐𝒕',
						 }
					} 
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`Andibot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
